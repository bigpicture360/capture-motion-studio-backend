...
def audio_track_source(track: dict[str, Any]) -> Any:
    if track.get("s3_uri") or track.get("s3Uri"):
        return track.get("s3_uri") or track.get("s3Uri")
    if track.get("url") or track.get("download_url") or track.get("downloadUrl"):
        return track.get("url") or track.get("download_url") or track.get("downloadUrl")
    s3_key = track.get("s3_key") or track.get("key") or track.get("path") or track.get("storage_path")
    if s3_key:
        return {
            "s3_key": s3_key,
            "bucket": track.get("bucket") or track.get("s3_bucket") or OUTPUT_BUCKET,
        }
    return None


def resolve_audio_tracks(settings: dict[str, Any]) -> list[AudioTrackSpec]:
    audio_settings = settings.get("audio") if isinstance(settings.get("audio"), dict) else {}
    audio_mix = settings.get("audio_mix") or audio_settings.get("audio_mix") or {}
    audio_enabled = truthy_setting(audio_settings.get("enabled")) or truthy_setting(audio_mix.get("enabled"))
    raw_tracks = (
        settings.get("audio_tracks")
        or audio_settings.get("audio_tracks")
        or audio_settings.get("tracks")
        or settings.get("soundtrack_tracks")
        or []
    )
    ...
    if not isinstance(raw_tracks, list):
        return []
    if not audio_enabled and not raw_tracks:
        return []

    tracks: list[AudioTrackSpec] = []
    for idx, raw_track in enumerate(raw_tracks):
        if isinstance(raw_track, str) and raw_track.strip():
            tracks.append(
                AudioTrackSpec(
                    source=raw_track.strip(),
                    title=Path(raw_track.strip()).name or f"audio-{idx + 1}",
                    volume=1.0,
                    start_at=0.0,
                    trim_start=0.0,
                    trim_end=None,
                    fade_in=0.0,
                    fade_out=max(0.0, safe_float(audio_settings.get("fade_out_seconds"), 0.0)),
                    loop=truthy_setting(audio_settings.get("loop_to_video_length")),
                )
            )
            continue
        if not isinstance(raw_track, dict):
            continue
        source = audio_track_source(raw_track)
        if not source:
            continue
        trim_end = raw_track.get("trim_end") or raw_track.get("trimEnd")
        tracks.append(
            AudioTrackSpec(
                source=source,
                title=str(raw_track.get("title") or raw_track.get("name") or raw_track.get("id") or f"audio-{idx + 1}"),
                volume=max(0.0, safe_float(raw_track.get("volume"), 1.0)),
                start_at=max(0.0, safe_float(raw_track.get("start_at") or raw_track.get("startAt"), 0.0)),
                trim_start=max(0.0, safe_float(raw_track.get("trim_start") or raw_track.get("trimStart"), 0.0)),
                trim_end=safe_float(trim_end, 0.0) if trim_end not in {None, ""} else None,
                fade_in=max(0.0, safe_float(raw_track.get("fade_in") or raw_track.get("fadeIn"), 0.0)),
                fade_out=max(0.0, safe_float(raw_track.get("fade_out") or raw_track.get("fadeOut") or audio_settings.get("fade_out_seconds"), 0.0)),
                loop=truthy_setting(raw_track.get("loop")) or truthy_setting(audio_settings.get("loop_to_video_length")),
            )
        )
    return tracks


def audio_ref_display_name(track: AudioTrackSpec, index: int) -> str:
    source = track.source
    if isinstance(source, dict):
        value = source.get("s3_key") or source.get("key") or source.get("path") or track.title
        return Path(str(value)).name or f"audio-{index}.m4a"
    return Path(str(source)).name or f"audio-{index}.m4a"


def download_audio_tracks(tracks: list[AudioTrackSpec], work_dir: Path) -> list[Path]:
    audio_dir = work_dir / "audio"
    audio_dir.mkdir(parents=True, exist_ok=True)
    audio_files: list[Path] = []
    for idx, track in enumerate(tracks, start=1):
        suffix = Path(audio_ref_display_name(track, idx)).suffix or ".m4a"
        dest = audio_dir / f"{idx - 1:03d}{suffix}"
        download_one_image(track.source, dest)
        if not dest.exists() or dest.stat().st_size == 0:
            raise RuntimeError(f"Downloaded audio is missing/empty: {dest}")
        audio_files.append(dest)
        log_event("audio_track_downloaded", index=idx, title=track.title, file=str(dest), size_bytes=dest.stat().st_size)
    return audio_files


def audio_mix_master_volume(settings: dict[str, Any]) -> float:
    audio_settings = settings.get("audio") if isinstance(settings.get("audio"), dict) else {}
    audio_mix = settings.get("audio_mix") or audio_settings.get("audio_mix") or {}
    return max(0.0, safe_float(audio_mix.get("master_volume") or audio_mix.get("masterVolume"), 1.0))


def build_audio_mux_command(video_path: Path, output_path: Path, tracks: list[AudioTrackSpec], audio_files: list[Path], settings: dict[str, Any], video_duration: float) -> list[str]:
    if not tracks or not audio_files:
        raise ValueError("No audio tracks to mux")

    cmd = [get_ffmpeg_bin(), "-hide_banner", "-y", "-i", str(video_path)]
    ...
    master_volume = audio_mix_master_volume(settings)
    for idx, track in enumerate(tracks):
        input_idx = idx + 1
        label = f"a{idx}"
        chain = [f"[{input_idx}:a]aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo"]
        trim_parts = [f"start={track.trim_start:.3f}"]
        if track.trim_end and track.trim_end > track.trim_start:
            trim_parts.append(f"end={track.trim_end:.3f}")
        chain.append(f"atrim={':'.join(trim_parts)}")
        chain.append("asetpts=PTS-STARTPTS")
        if track.loop:
            chain.append("aloop=loop=-1:size=2147483647")
        effective_volume = track.volume * master_volume
        chain.append(f"volume={effective_volume:.4f}")
        if track.start_at > 0:
            delay_ms = int(round(track.start_at * 1000))
            chain.append(f"adelay={delay_ms}:all=1")
        chain.append(f"atrim=0:{duration:.3f}")
        if track.fade_in > 0:
            chain.append(f"afade=t=in:st=0:d={min(track.fade_in, duration):.3f}")
        if track.fade_out > 0:
            fade_duration = min(track.fade_out, duration)
            fade_start = max(0.0, duration - fade_duration)
            chain.append(f"afade=t=out:st={fade_start:.3f}:d={fade_duration:.3f}")
    ...
    if len(audio_labels) == 1:
        ...
        filters.append("".join(f"[{label}]" for label in audio_labels) + f"amix=inputs={len(audio_labels)}:duration=longest:dropout_transition=0,volume=1.0[aout]")
...
def min_motion_zoom(effect: str) -> float:
    if effect in {"whip_pan"}:
        return 1.22
    if effect in {"diagonal_pan"}:
        return 1.16
    if effect in {"pan_left", "pan_right", "pan_up", "pan_down", "pan", "pan_zoom"}:
        return 1.14
    if effect in {"handheld_drift", "rotation_drift"}:
        return 1.12
    return 1.0


def safe_zoom_expr(zoom: str, min_zoom: float = 1.0) -> str:
    value = str(zoom).strip().strip("'")
    if min_zoom <= 1.0:
        return f"'{value}'"
    return f"'max({min_zoom:.4f},({value}))'"


def safe_crop_expr(expr: str, axis: str) -> str:
    value = str(expr).strip().strip("'")
    extent = "iw-iw/zoom" if axis == "x" else "ih-ih/zoom"
    return f"'floor(min(max(0,({value})),max(0,{extent})))'"
...
def motion_filter_for_effect(
    ...
    if effect == "pull_out":
        ...
        zoom = f"'1.08+0.06*({progress})'"
        x = f"'iw/2-(iw/zoom/2)+10*sin(2*PI*on/{n})'"
        y = f"'ih/2-(ih/zoom/2)+7*cos(2*PI*on/{n})'"
    elif effect == "handheld_drift":
        zoom = "'1.08'"
        x = f"'iw/2-(iw/zoom/2)+8*sin(on/9)'"
        y = f"'ih/2-(ih/zoom/2)+6*cos(on/11)'"

    zoom = safe_zoom_expr(zoom, min_motion_zoom(effect))
    x = safe_crop_expr(x, "x")
    y = safe_crop_expr(y, "y")
...
def motion_filter_for_effects(
    ...
    for motion_idx, motion in enumerate(motions):
        ...
        safe_min_zoom = max(safe_min_zoom, min_motion_zoom(effect))
    ...
    zoom = safe_zoom_expr(zoom, safe_min_zoom)
    x = safe_crop_expr(x, "x")
    y = safe_crop_expr(y, "y")
...
def run_audio_mux_command(cmd: list[str], job_id: str, timeout_seconds: int) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Audio mux timed out after {round(time.time() - started_at, 2)}s") from exc
    log_event("audio_mux_finished", job_id=job_id, returncode=result.returncode, elapsed_seconds=round(time.time() - started_at, 2))
    if result.returncode != 0:
        raise RuntimeError(f"Audio mux failed with return code {result.returncode}; stderr_tail={(result.stderr or '')[-4000:]}")
...
def render_job(job: RenderJob, receipt_handle: Optional[str] = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"render-{job.job_id[:8]}-", dir=str(RENDER_WORK_DIR)) as tmp:
        work_dir = Path(tmp)
        audio_tracks = resolve_audio_tracks(render_settings)
        audio_files: list[Path] = []
        if audio_tracks:
            reporter.emit(stage="downloading", status="processing", progress_percent=2, message="Downloading audio track", force=True)
            audio_files = download_audio_tracks(audio_tracks, work_dir)
        ...
        video_only_path = work_dir / "render_video_only.mp4" if audio_tracks else output_path
        cmd, image_stage_seconds = build_ffmpeg_command(processed_files, video_only_path, render_settings, job.per_image_settings)
        try:
